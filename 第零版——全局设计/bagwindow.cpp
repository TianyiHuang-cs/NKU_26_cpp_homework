...
// 添加物品到背包（最多9格）
// ============================================================
void BagWindow::addItem(int id, const QString &name, const QPixmap &icon)
{
    for (const BagItem &item : m_items)
        if (item.id == id) return;
    if (m_items.size() >= 9) return;

    BagItem newItem;
    newItem.id   = id;
    newItem.name = name;
    newItem.icon = icon.scaled(CELL_SIZE - 16, CELL_SIZE - 16,
