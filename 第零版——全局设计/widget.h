...
class ThirdFloor;
class FourthFloor;
class IntroAnimation;  // 开始故事动画
class OutroAnimation;  // 结束故事动画
...
public slots:
    // 触发结束动画（由 ThirdFloor/FourthFloor 发出 gameEnd 信号后调用）
    void startOutro();
...
private:
    ...
    // ===== 动画 =====
    IntroAnimation *m_intro;
    OutroAnimation *m_outro;
