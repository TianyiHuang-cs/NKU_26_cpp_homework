...
// 放入物品：记录 ID 和图标，触发重绘
// ============================================================
void SlotWidget::setItem(int itemId, const QPixmap &icon)
{
    m_itemId = itemId;
    m_icon   = icon;
    update();
...
// ============================================================
void GramophoneWindow::placeItemIntoSlot(int slotIndex, int itemId, const QPixmap &icon)
{
    if (slotIndex < 0 || slotIndex > 2) return;
    m_slots[slotIndex]->setItem(itemId, icon);
    m_resultLabel->hide(); // 重新放置物品后隐藏上次结果
}
...
void GramophoneWindow::onConfirmClicked()
{
    if (checkAnswer()) {
        ...
        m_resultLabel->setText("♪ 心声最重要——空余时间 + 10min");
        m_resultLabel->show();
